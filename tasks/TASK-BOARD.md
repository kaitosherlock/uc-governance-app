...
| P0-05 | `src/api/client.ts`: the only `fetch`; prefixes `/api/v1`, URL-encodes `full_name`, parses the envelope, narrows on `success`, throws typed `ApiError`, forwards `AbortSignal`, treats 202 as `Operation.status === 'unknown'`. Types come from `@contracts/types`, nothing is generated from a running server. Plus MSW handlers built from `shared/contracts/examples/` with `?scenario=` variants | antigravity | P0-04 | §6, §12 | in_progress | 2026-09-22 | **Gate 3 FAILED, orchestrator-verified; the agent's own `done` was corrected.** eslint exits 0, but `tsc --noEmit` reports 1 error (client.ts:91, TS2379 under exactOptionalPropertyTypes) and vitest reports 10 of 38 tests failing because no vitest config selects the jsdom environment, so `document` is undefined and root-relative URLs cannot be parsed. Rejection queued at `.ai/prompts/P0-05-gate3-reject.md`. Authored `frontend/src/lib/strings.ts` (added all 21 errorCodes, context labels), `frontend/src/api/errors.ts` (ApiError, AbortError, describeError, isClientError), `frontend/src/api/client.ts` (only fetch, buildUrl encoding, 202 Operation handling, AbortSignal handling), `frontend/src/api/queries.ts` (useAppContext, useContext, useMe, useCapabilities with identity-scoped keys and 4xx retry disabling), `frontend/src/mocks/handlers.ts` (verbatim examples + ?scenario= forbidden, not-configured, validation, unknown-outcome, slow), `frontend/src/mocks/browser.ts`, `frontend/src/mocks/enable.ts` (dev-only conditional MSW start), `frontend/src/app/providers.tsx` (QueryClientProvider), `frontend/src/main.tsx` (enableMocking before render), `frontend/src/app/ContextBar.tsx` (real loading/error/limitations states), `frontend/src/app/AppShell.tsx` (wired to real hooks). Unit tests in `client.test.ts`, `errors.test.ts`, and `ContextBar.test.tsx`. |
