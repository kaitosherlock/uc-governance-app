...
def test_sdk_effective_sources_are_classified_without_guessing() -> None:
    ...
    inherited = map_source(
        SimpleNamespace(inherited_from_type="SCHEMA", inherited_from_name="sales.crm"),
        "TABLE",
        "sales.crm.orders",
    )
...
def test_direct_and_effective_sdk_privileges_map_to_exact_codes() -> None:
    direct = SimpleNamespace(
        privilege_assignments=[SimpleNamespace(principal="analysts", privileges=["SELECT"])]
    )
...
def test_fixture_grants_disclose_membership_and_parent_routes(client) -> None:
    ...
    assert {g["allowed_actions"][0]["navigate_to"] for g in inherited} == {
        "/assets/sales?tab=access",
        "/assets/sales/crm?tab=access",
    }
