...
from app.domain.models import AssetDetail, AssetSummary, DependenciesData, Grant, GrantSource, Principal
...
class FixtureReaders:
    ...
    def list_catalogs(self, page_size: int, page_token: str | None) -> tuple[list[AssetSummary], str | None]:
        values: list[AssetSummary] = [a for a in self.assets.values() if a.securable_type == SecurableType.CATALOG]
    ...
    def list_schemas(self, catalog: str, page_size: int, page_token: str | None) -> tuple[list[AssetSummary], str | None]:
        ...
        values: list[AssetSummary] = [a for a in self.assets.values() if a.securable_type == SecurableType.SCHEMA and a.parent and a.parent.full_name == catalog]
    ...
    def list_objects(self, catalog: str, schema: str, page_size: int, page_token: str | None) -> tuple[list[AssetSummary], str | None]:
        ...
        values: list[AssetSummary] = [a for a in self.assets.values() if a.parent and a.parent.full_name == f'{catalog}.{schema}']
    ...
    def direct_grants(self, securable_type: str, full_name: str, page_size: int, page_token: str | None) -> tuple[list[Grant], str | None]:
        self.get_asset(securable_type, full_name)
        return page(self.grants.get((securable_type, full_name), []), page_size, page_token, f'direct:{full_name}')

    def effective_grants(self, securable_type: str, full_name: str, page_size: int, page_token: str | None) -> tuple[list[Grant], str | None]:
        asset = self.get_asset(securable_type, full_name)
        values = list(self.grants.get((securable_type, full_name), []))
        while asset.parent:
            parent = asset.parent
            values.extend(replace(g, source=replace(g.source, type=GrantSourceType.INHERITED))
    ...
    def search_principals(self, query: str, kinds: tuple[str, ...], page_size: int, page_token: str | None) -> tuple[list[Principal], str | None]:
        values = [p for p in self.principals if query.casefold() in p.display.casefold() and (not kinds or p.kind in kinds)]
    ...
    def dependencies(self, securable_type: str, full_name: str) -> DependenciesData:
        self.get_asset(securable_type, full_name)
        return DependenciesData(known=DEPENDENCIES.get(full_name, ()), disclaimer='Missing dependency information is not proof that deletion is safe.')

    def update_grants(self, securable_type: str, full_name: str, principal: str, add: tuple[str, ...], remove: tuple[str, ...]) -> None:
        self.get_asset(securable_type, full_name)
        ...
        current = [g for g in self.grants.get(key, []) if not (g.principal == principal and g.privilege in remove)]
        for privilege in add:
            if not any(g.principal == principal and g.privilege == privilege for g in current):
                current.append(Grant(principal=principal, principal_kind=person.kind, privilege=privilege,
                    source=GrantSource(type=GrantSourceType.DIRECT, securable_type=SecurableType(securable_type), full_name=full_name), allowed_actions=()))
