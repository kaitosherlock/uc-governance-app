...
from app.adapters.databricks.common import CursorStore, boundary
...
if TYPE_CHECKING:
    ...
    from databricks.sdk.service.catalog import CatalogsAPI, FunctionsAPI, GrantsAPI, RegisteredModelsAPI, SchemasAPI, TablesAPI, VolumesAPI
...
class ObjectsRouter:
    ...
    def list_objects(self, catalog: str, schema: str, page_size: int, page_token: str | None) -> tuple[list[AssetSummary], str | None]:
        ...
        token = f'{index}:{next_inner}' if next_inner else f'{index+1}:' if index+1 < len(self.readers) else None
        return values, token
...
class SDKIdentityResolver:
    ...
    async def resolve(self, access_token: str) -> ResolvedUser:
        ...
        try:
            from databricks.sdk import WorkspaceClient
            client = WorkspaceClient(host=self.settings.workspace_host, token=access_token, auth_type='pat')
        ...
        return ResolvedUser(actor=Actor(id=user.id, display=user.user_name, kind=ActorKind.USER,
            roles=[AppRole.VIEWER], verified_by='user_token'), email=user.user_name)
...
def sdk_readers(settings: Settings, access_token: str, cursors: CursorStore) -> Readers:
    ...
    def client(executor: str) -> 'WorkspaceClient':
        if executor not in clients:
            from databricks.sdk import WorkspaceClient
            clients[executor] = WorkspaceClient(host=settings.workspace_host, token=access_token, auth_type='pat') if executor == 'user' else WorkspaceClient(host=settings.workspace_host)
    ...
    models = ModelsAdapter(cast('RegisteredModelsAPI', service('registered_models', 'sp')), cursors, key)
    grants = GrantsAdapter(cast('GrantsAPI', service('grants', 'sp')))
    principals = PrincipalsAdapter(cast('UsersAPI', service('users', 'sp')), cast('GroupsAPI', service('groups', 'sp')), cast('ServicePrincipalsAPI', service('service_principals', 'sp')), cursors, key)
    from databricks.sdk.service.catalog import Privilege
    return Readers(catalogs=catalogs, schemas=schemas, objects=ObjectsRouter((tables, volumes, functions, models)),
        assets=AssetRouter({'CATALOG': catalogs, 'SCHEMA': schemas, 'TABLE': tables, 'VOLUME': volumes, 'FUNCTION': functions, 'REGISTERED_MODEL': models}),
        grants=grants, principals=principals, dependencies=DependenciesAdapter(),
