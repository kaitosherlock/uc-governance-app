...
def map_table(value: object) -> AssetDetail:
    ...
    kinds = {
        "MANAGED": ObjectKind.TABLE,
        "EXTERNAL": ObjectKind.TABLE,
        "MANAGED_SHALLOW_CLONE": ObjectKind.TABLE,
        "EXTERNAL_SHALLOW_CLONE": ObjectKind.TABLE,
        "FOREIGN": ObjectKind.TABLE,
        "VIEW": ObjectKind.VIEW,
        "MATERIALIZED_VIEW": ObjectKind.MATERIALIZED_VIEW,
        "STREAMING_TABLE": ObjectKind.STREAMING_TABLE,
    }
    if table_type not in kinds:
        raise NotImplementedYet(
            "This SDK table type cannot be represented by the current object-kind catalogue."
        )
    ...
    for item in items_field(value, "columns"):
        ...
        columns.append(
            Column(
                ...
                comment=text_field(item, "comment"),
                position=position,
                tags=(),
                mask=ColumnMaskRef(
                    column=name,
                    ...
                    using_columns=tuple(
                        x for x in items_field(mask, "using_column_names") if isinstance(x, str)
                    ),
                    attached_via=AttachmentSource.UNKNOWN,
                )
                if function
                else None,
            )
    ...
    return replace(
        ...
        data_source_format=enum_field(value, "data_source_format"),
        properties=safe_props,
        columns=tuple(columns),
        row_filter=RowFilterRef(
            function_full_name=function,
            input_columns=tuple(
                x for x in items_field(row, "input_column_names") if isinstance(x, str)
            ),
            attached_via=AttachmentSource.UNKNOWN,
        )
        if function
...
class TablesAdapter:
    ...
    def list_objects(
        ...
        values, token = self.cursors.page(
            ...
            f"{self.identity_key}:tables:{catalog}:{schema}",
            page_size,
            page_token,
        )
