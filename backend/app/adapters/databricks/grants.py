...
def map_source(value: object, stype: str, name: str) -> GrantSource:
    ...
    if parent_type is None and parent_name is None:
        return GrantSource(type=GrantSourceType.DIRECT, securable_type=SecurableType(stype), full_name=name)
    if parent_type in ('CATALOG', 'SCHEMA') and parent_name:
        return GrantSource(type=GrantSourceType.INHERITED, securable_type=SecurableType(parent_type), full_name=parent_name)
...
def map_grants(value: object, stype: str, name: str, effective: bool) -> list[Grant]:
    ...
    for assignment in items_field(value, 'privilege_assignments'):
        ...
        for privilege in items_field(assignment, 'privileges'):
            code = enum_field(privilege, 'privilege') if effective else enum_field(assignment_code(privilege), 'code')
            if code is None:
                raise UpstreamUnavailable('Databricks returned incomplete privilege metadata.')
            source = map_source(privilege, stype, name) if effective else GrantSource(type=GrantSourceType.DIRECT, securable_type=SecurableType(stype), full_name=name)
            grants.append(Grant(principal=principal, principal_kind=None, privilege=code, source=source, allowed_actions=()))
    return grants
...
class GrantsAdapter:
    ...
    def direct_grants(self, securable_type: str, full_name: str, page_size: int, page_token: str | None) -> tuple[list[Grant], str | None]:
        value = self.api.get(securable_type=securable_type, full_name=full_name, max_results=min(200, max(1, page_size)), page_token=page_token)
    ...
    def effective_grants(self, securable_type: str, full_name: str, page_size: int, page_token: str | None) -> tuple[list[Grant], str | None]:
        value = self.api.get_effective(securable_type=securable_type, full_name=full_name, max_results=min(200, max(1, page_size)), page_token=page_token)
