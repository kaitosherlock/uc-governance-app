...
def map_volume(value: object) -> AssetDetail:
    ...
    status = {"MANAGED": ManagedStatus.MANAGED, "EXTERNAL": ManagedStatus.EXTERNAL}.get(
        enum_field(value, "volume_type") or "", ManagedStatus.UNKNOWN
    )
...
class VolumesAdapter:
    ...
    def list_objects(
        ...
        values, token = self.cursors.page(
            ...
            f"{self.identity_key}:volumes:{catalog}:{schema}",
            page_size,
            page_token,
        )
