...
def translate_exception(exc: Exception) -> AppError:
    ...
    code = possible_code if isinstance(possible_code, str) else None
    ...
    mapping = {
        401: (ErrorCode.INSUFFICIENT_PRIVILEGES, 403),
        403: (ErrorCode.INSUFFICIENT_PRIVILEGES, 403),
    ...
    if request_id is None:
        error_details = getattr(exc, "get_error_details", None)
        if callable(error_details):
            request_info = getattr(error_details(), "request_info", None)
...
def base_asset(value: object, stype: SecurableType, kind: ObjectKind) -> AssetDetail:
    ...
    if len(bits) > 1:
        ...
        parent_name = (
            ...
            if "`" not in full_name
            else ".".join("`" + p.replace("`", "``") + "`" for p in bits[:-1])
        )
        parent = AssetRef(
            securable_type=SecurableType.CATALOG if len(bits) == 2 else SecurableType.SCHEMA,
            full_name=parent_name,
            kind=ObjectKind.CATALOG if len(bits) == 2 else ObjectKind.SCHEMA,
            display_name=bits[-2],
        )
    return AssetDetail(
        ...
        full_name=full_name,
        kind=kind,
        display_name=text_field(value, "name") or bits[-1],
        ...
        created_by=text_field(value, "created_by"),
        parent=parent,
        raw={},
    )
...
class CursorStore:
    ...
    @boundary
    def page(
        self, factory: Callable[[], Iterator[object]], key: str, size: int, token: str | None
            ...
            if token:
                ...
                if entry is None or entry[1] != key:
                    raise ValidationFailed(
                        "The page token expired or belongs to another listing. Restart the listing."
                    )
