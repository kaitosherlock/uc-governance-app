...
READ_ACTIONS = frozenset({
    "context.read", "identity.read", "capabilities.read", "assets.read", "catalogs.read",
    "schemas.read", "principals.read", "grants.read", "privileges.read", "tags.read",
    "tag_policies.read", "classification.read", "abac_policies.read", "filters.read",
    "storage.read", "bindings.read", "federation.read", "sharing.read", "lineage.read",
    "activity.read", "audit.read", "findings.read", "quality.read", "models.read",
    "serving_endpoints.read", "requests.read", "reviews.read", "plans.read", "operations.read",
})
STEWARD_ACTIONS = frozenset({
    "edit_metadata", "assign_tags", "remove_tags", "transfer_ownership", "review_findings",
})
ACCESS_ACTIONS = frozenset({
    "grant", "revoke", "transfer_ownership", "approve", "access_review", "apply_access_request",
})
PLATFORM_ACTIONS = STEWARD_ACTIONS | ACCESS_ACTIONS | frozenset({
    "set_row_filter", "drop_row_filter", "set_column_mask", "drop_column_mask",
    "replace_view_definition", "delete_asset", "update_binding", "create_abac_policy",
    "update_abac_policy", "delete_abac_policy", "update_share_permissions", "update_recipient",
    "create_quality_monitor", "refresh_quality_monitor", "storage.create", "storage.update",
    "storage.delete", "federation.create", "federation.update", "federation.delete",
    "sharing.create", "sharing.update", "sharing.delete", "admin.read", "admin.update",
})
...
def roles_for_groups(
    groups: Iterable[str] | None, role_groups: Mapping[str, list[str]],
) -> list[AppRole]:
...
def decide(
    identity: Identity, action: str, target: Target, *,
    managed_catalogs: Iterable[str] = (), mode: Mode = Mode.FIXTURE,
) -> Decision:
